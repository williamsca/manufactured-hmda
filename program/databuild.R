...
if (!dir.exists(here("data", "hmda"))) {
    stop("HMDA data directory not found. Please run data retrieval scripts first.")
}
...
# Data Import ----
# Import HMDA files (should be generated by data retrieval scripts)
l_hmda <- list.files(
    here("data", "hmda"),
    pattern = "hmda_\\d{4}\\.csv", full.names = TRUE
)
if (length(l_hmda) == 0) {
    stop("No HMDA files found. Please run data retrieval scripts to download HMDA data.")
}
...
cpi_files <- list.files(here("data", "cpi"), pattern = "\\.xlsx$", full.names = TRUE)
